...
function buildIdToTagMapFromSameSheet(ws: ExcelJS.Worksheet): Map<string, string> {
  ...
  for (let r = 1; r <= maxR; r++) {
    ...
    if (v && /ID\s*e\s*Nome/i.test(v)) { headerRow = r; break; }
  }
  ...
  if (headerRow > 0) {
    for (let r = headerRow + 1; r <= maxR; r++) {
      ...
      for (let c = 1; c <= maxC; c++) {
        ...
        if (m) { idToTag.set(m[1], m[2]); break; }
      }
    }
...
function processRow(values: any[], leafIds: Map<number, string>, idToPath: Map<string, string[]>) {
    ...
    if (maxLen > 1 && pref.split(".").length > 0) {
      ...
      if (!groupPath || groupPath.length === 0) {
        ...
        if (sample && sample.length > 1) {
          ...
          // No podemos determinar un contenedor seguro -> no agrupamos,
          // estos campos se renderizan luego como "singles".
          continue;
        }
      }

      // 2) Tag del grupo y ruta del padre; si no existe, salimos
  ...
  let header: any = null, body: any = null;
  ...
  if (header && body) {
    (root as any).remove(header); (root as any).remove(body);
    (root as any).import(header); (root as any).import(body);
  }
  if (header) header.att("xmlns", "");
  if (body) body.att("xmlns", "");

  // asegurar DatiGeneraliDocumento primero
  if (body) {
...
export async function excelToXmlFromSingleXlsx(
  ...
  for (let r = 2; r <= ws.actualRowCount; r++) {
    ...
    const nonEmpty = values.filter((v) => {
      const s = cellStr(v); return s && s.trim();
    });
